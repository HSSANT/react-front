  ...
  .actions((self) => ({
    ...
    newPurchase: flow(function* (transaction: Transaction) {
      ...
      if (result.kind === "ok") {
        ...
        self.setStatus("error")
        __DEV__ && console.tron.log(result.kind)
        if (result.kind == "token-expired") return { ...result }
      }
    }),

    transactionsFiltered: flow(function* (userId: number, date: Date) {
      self.setStatus("pending")

      const transactionApi = new TransactionApi(self.environment.api)
      const result: TransactionsResult = yield transactionApi.transactionsFiltered(userId, date)
      if (result.kind === "ok") {
        self.setStatus("done")
    ...
    transactionUpdate: flow(function* (id: number, authorized: boolean) {
      self.setStatus("pending")

      const transactionApi = new TransactionApi(self.environment.api)
      const result: TransactionResult = yield transactionApi.transactionUpdate(id, authorized)
      if (result.kind === "ok") {
        self.setStatus("done")
        return result.transaction
      } else {
