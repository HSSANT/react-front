...
export const AuthenticationStoreModel = types
  ...
  .actions((self) => ({
    login: flow(function* (
      ...
      hasRequestError?: () => void,
    ) {
      self.setStatus("pending")

      const authenticationApi = new AuthenticationApi(self.environment.api)
      const result: LoginResult = yield authenticationApi.login(email, password)
      if (result.kind === "ok") {
        setUser(result.response.data.user)
        saveString("id", result.response.data.user.id.toString())
        saveString("userName", result.response.data.user.name)
        saveString("accessToken", result.response.data.access_token)
        result?.response?.data?.user?.roles[0].name === "administrator"
    ...
    register: flow(function* (email: string, password: string, userName: string) {
      self.setStatus("pending")

      const authenticationApi = new AuthenticationApi(self.environment.api)
      const result: RegisterResult = yield authenticationApi.register(email, password, userName)
